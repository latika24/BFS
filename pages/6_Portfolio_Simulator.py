"""Portfolio Simulator — the book from year 1 to 7. §4, §4.1, §2.1."""
...
from shared import (page_setup, page_header, kpi, inr, crore, note, note_ref,
...
page_header(
    "Portfolio Simulator",
    f"{SEC['monetization']} · {SEC['breakeven']}",
    "The book from year 1 to year 7 — premium, loss ratio, expense ratio, "
    "solvency and capital. Year 1 is the first full year of writing, project "
    "month 21 onward.")
...
if scenario == "Loss ratio plateaus at 78%":
    ...
    sb.caption("Fraud on income-benefit claims proves harder to control than "
               "modelled. §4.1, first failure mode.")
elif scenario == "Growth stalls at 7 lakh":
    growth = 700000 / CFG["portfolio"]["trajectory"][-1]["workers"]
    sb.caption("The Zego failure mode (§2.1): the expense ratio never falls "
               "because premium never reaches scale. §4.1, second failure mode.")
...
if be is None:
    st.error("**This scenario never reaches underwriting profit.** The book "
             ...
             "— the plan's response is in §4.1: extend to ride-hail and bike-taxi "
...
st.info(
    ...
    "international benchmark in §2.1 teaches: their loss ratio was excellent "
...
with r:
    ...
    note_ref("§4 — burning cost build-up, per 1,000 full-time riders")
...
cap = portfolio.capital_plan()[["name", "year", "amount_cr", "underwriting"]].copy()
cap["amount_cr"] = cap["amount_cr"].map(lambda v: f"₹{v:,.0f} cr")
cap.columns = ["Round", "Year", "Amount", "What the investor underwrites"]
...
note(f"<b>A discrepancy in the report.</b> {CFG['portfolio']['funding_note']}")
